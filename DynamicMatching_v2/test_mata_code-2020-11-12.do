...
pointer matrix main()
{
	...
	s.p_w = &(*s.by:*1 :+ !*s.by:*(*s.p_haz):/(1:-*s.p_haz) )
// 	s.p_w = &.
	s.wtype = "iweight"
	

// 	s.p_haz = &logitreg(*s.by,*s.p_X)
	s.p_w = &(*s.by:*1 :+ !*s.by:*(*s.p_haz):/(1:-*s.p_haz) )
...
// 		`RV' index
		...
		(*s.p_w) = *s_dm.treated :* 1 :+ !*s_dm.treated:*(*s_dm.p_haz):/(1:-(*s_dm.p_haz)) //  J(rows(index),1,1) 
		s.wtype = "iweight"
		if (*s.p_w != . )
		{
			index = selectindex(!*s_dm.treated)
