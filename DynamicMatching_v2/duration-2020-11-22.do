...
struct duration {
	...
	`RS' failcode, weightData, curr_timeToTreat 
		   ...
		   `SS' wtype)
{
	...
	`RM' means, sd 
	k = cols(data)
	means = sd = J(group.num_levels,k,.)

	for ( i = 1 ; i <= k ; i++)
	{
		means[,i] = aggregate_mean(group,data[,i],weights,wtype)
		sd[,i]    = aggregate_sd(group,data[,i],.,"") // aggregate_sd(s.group,data[,i],*s.p_w,s.wtype)
	...
	m1 = select(means,group.keys:==1)
	m0 = select(means,group.keys:==0)
	sd1 = select(sd  , group.keys:==1)
	sd0 = select(sd  , group.keys:==0)
	dif = (m1 - m0)
	bias = 100*dif:/sqrt(0.5*(sd1:+sd0))
	
	return( (m1\m0\dif\bias)')
