...
// 	time_treatment = uniqrows(select(timeToTreatment,*s.by:==1))
	nTimesTreat = 2 // length(time_treatment)
	...
	real scalar Ntimes 
	Streat = weights = Scontrols = J(s.t.num_levels,nTimesTreat+1,.)
	Ntreat = ftimeTreat.counts[1..nTimesTreat]
...
// 		colmin(select(data,data[,cols(data)-1]:==1))
		...
		Scontrols[res[,1],i] = res[,2]*wcontrols[i]
