...
replace cause = cond( u_x <= p1 , 2 , 3 ) if inProg & failure
...
br id t trans inProg cause failure u F u_x p* exit  haz* sumHaz _exit 

// br id t trans inProg _exit if trans == 1

bys id (t) : egen byte treated = sum(trans==1)
