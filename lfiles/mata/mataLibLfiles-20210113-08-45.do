global pado P:\Data\SFI_adm_704144\VIVE_Programmer\Stata\ado
adopath + $pado
// mata: mata mlib index

cd `c(sysdir_personal)'
// cap erase llfiles.mlib
...
void listDir(string scalar directory,string scalar ext, 
             real scalar showDir)
{
...
// 		printf("{text}Current directory      : {res}%s\n",pwd())
		...
		printf("{text}Root directory         : {res}%s\n",rootDirectory)
		display(linksToPath(pathname))
		
		string scalar str 
		str = st_global("c(os)") == "Windows" ? sprintf(`"{col 40}{stata "winexec explorer "') + st_global("c(pwd)") + `"":Explorer}"' : ""
// 		display(str)
		display(`"{stata "lfiles , cls":Clear screen}"' + str)
		display(`"{stata "lfiles .. , nof":..}"') // + rootDirectory + `" , nof cd ":..}"')
		display(`"{stata "lfiles "' + pathname + `" , cd ":.}"')
		
		if (showDir) showDir(pathname)
...
// 		exit()
		if (rows(dirs)==0) printf("No subdirectories found\n")
            ...
            for (i  = 1; i <= rows(dirs) ; i++)
            {
                    ...
                    dirname = dir + "\*"
                    toBeDisplayed = `"{text}<dir>"' + buttonAddPath(dir) + 
                    buttonStataCmd("winexec explorer ",dir,"[E]") +
                    `" {stata "lfiles "' + dirname + `" , cd nof":"' +dirs[i]+ "}"
...
include "P:\Data\SFI_adm_704144\VIVE_Programmer\Stata\ado\lf_mata_favorites_20210107.do"
...
//  C:\Users\yds4144\ado\personal\ , replace 
copy P:\Data\SFI_adm_704144\VIVE_Programmer\Stata\ado\lfiles.sthlp ///
 C:\Users\yds4144\ado\personal\ , replace 
...
cd R:\__econometrie\stata\lfiles
adopath + R:\__econometrie\stata\lfiles
